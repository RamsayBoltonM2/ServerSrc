...
void CItemCache::OnFlush()
{
	...
	else
	{
		...
		int iLen = snprintf(szColumns, sizeof(szColumns), "id, owner_id, window, pos, count, vnum, evolution, transmutation");
		int iValueLen = snprintf(szValues, sizeof(szValues), "%u, %u, %d, %d, %u, %u, %d, %u", p->id, p->owner, p->window, p->pos, p->count, p->vnum, p->evolution, p->transmutation);
		int iUpdateLen = snprintf(szUpdate, sizeof(szUpdate), "owner_id=%u, window=%d, pos=%d, count=%u, vnum=%u, evolution=%d, transmutation=%u", p->owner, p->window, p->pos, p->count, p->vnum, p->evolution, p->transmutation);
