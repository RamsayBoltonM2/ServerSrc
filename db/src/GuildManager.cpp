...
{
#ifdef __IMPROVED_GUILD_WAR__
	decltype(m_DeclareMap.begin()) it = m_DeclareMap.find(TGuildDeclareInfo(0, guild_from, guild_to, iMaxPlayer, iMaxScore, flags, custom_map_index));
#else
...
void CGuildManager::BootReserveWar()
{
	...
	for (int i = 0; i < 2; ++i)
	{
		...
		while ((row = mysql_fetch_row(pmsg->Get()->pSQLResult)))
		{
			...
			char buf[512];
#ifdef __IMPROVED_GUILD_WAR__
			snprintf(buf, sizeof(buf), "GuildWar: BootReserveWar : step %d id %u GID1 %u GID2 %u max player %d max score %d flags %ld custommapidx %d", i, t.dwID, t.dwGuildFrom, t.dwGuildTo, t.iMaxPlayer, t.iMaxScore, t.flags, t.custom_map_index);
#else
...
bool CGuildManager::ReserveWar(TPacketGuildWar * p)
{
	...
	snprintf(szQuery, sizeof(szQuery),
			...
			"VALUES(%u, %u, DATE_ADD(NOW(), INTERVAL 180 SECOND), %u, %ld, %ld, %ld, %ld, %ld, %ld, %ld)",
