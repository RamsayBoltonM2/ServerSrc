	...
	if (pkSk->dwType != 0)
	{
		...
		switch (GetSkillMasterType(pkSk->dwVnum))
		{
			case SKILL_NORMAL:
				...
				{
					if (GetQuestFlag("reset_scroll.force_to_master_skill") <= 100)
					{
...
#if defined(__YMIR_UPDATE_WOLFMAN__) && defined(USE_ITEM_CLAW_AS_DAGGER)
								iDam = iDam * (100 - pkChrVictim->GetPoint(POINT_RESIST_DAGGER)) / 100;
#endif
								break;
#endif
...
{
#ifdef __SKILL_MANA_FIX_UPDATE__
	LPITEM pWeapon = GetWear(WEAR_WEAPON);
	int skillIndex = 63; // Skill sura -> Tais Vrajit
	
	if ((dwVnum == SKILL_GEOMKYUNG || dwVnum == SKILL_GWIGEOM) && !pWeapon)
	{
		ChatPacket(CHAT_TYPE_NOTICE, "<<Debug>> You try to use skillIndex %d and you not have weapon equipped.", dwVnum);
		return false;
	}
#endif
#ifdef __ENABLE_PVP_ADVANCED__
	int getSkill[] = {95, 94, 96, 109, 110, 111};
	
	for (int j = 0; j < _countof(getSkill); j++)
	{
		if (getSkill[j] == dwVnum && pkVictim->IsPC() && pkVictim->GetQuestFlag(BLOCK_BUFF))
		{
...
#if defined(__YMIR_UPDATE_WOLFMAN__) && !defined(USE_WOLFMAN_BOOKS)
	...
	if (true == IsRiding())
	{
		if(GetMountVnum())
		{
			if( !((GetMountVnum() >= 20209 && GetMountVnum() <= 20212)	||
				GetMountVnum() == 20215 || (GetMountVnum() >= 20218 && GetMountVnum() <= 20222) || GetMountVnum() == 20225 ||  (GetMountVnum() >= 20107 && GetMountVnum() <= 20132)	 ||  (GetMountVnum() >= 20226 && GetMountVnum() <= 20235)	)	)
					return false;
		}
