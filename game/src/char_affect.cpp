...
{
#ifdef ENABLE_TEST_FIX_TAIS	
	if (!GetWear(WEAR_WEAPON))
	{
		if (IsAffectFlag(AFF_GEOMGYEONG))
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT ("Buyulu Keskinlik becerisini el degmemis bir silah olmadan harekete geciremezsiniz."));
			RemoveAffect(SKILL_GEOMKYUNG);
		}
		
		if (IsAffectFlag(AFF_GWIGUM))
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT ("Hava Kilici becerisini el degmemis bir silah olmadan harekete geciremezsiniz."));
