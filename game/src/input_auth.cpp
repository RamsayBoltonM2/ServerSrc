...
void CInputAuth::Login(LPDESC d, const char * c_pData)
{
	...
	if (pinfo->security_key[0] != 1999 || pinfo->security_key[1] != 11 || pinfo->security_key[2] != 5 || pinfo->security_key[3] != 19 || pinfo->security_key[4] || 0)
	{
		TPacketGCLoginFailure failurePacket;

		failurePacket.header = HEADER_GC_LOGIN_FAILURE;
		strlcpy(failurePacket.szStatus, "BLOCK_SE", sizeof(failurePacket.szStatus));
