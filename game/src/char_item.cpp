...
void CHARACTER::__OpenOfflineShop()
{
	...
	if (IsAcceOpen())
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("OFFLINE_SHOP_ACCE_AKTIF"));
		return;		
	}
...
#ifdef __AFFECT_POTION_UPDATE__
bool CHARACTER::SetAffectPotion(LPITEM item)
{
   ...
   // const char* blend_succes = {"<Affect Potion> Set icon ingame for item: |cFFc9ff00|H|h[%s]"};
	ChatPacket(CHAT_TYPE_INFO, "Sebnem icildi.."); 
...
bool CHARACTER::CheckTimeUsed(LPITEM item)
{
	switch (item->GetVnum())
	{
		...
		case 76003:	case 76017:	case 76018: case 50815: case 50816:
			...
			const char* pGetMessage[] = {"Bu esyayi oyuna girdikten|cFFd0ffcc|H|h |cFFec03e2|H|h[%u] |cFFd0ffcc|H|h saniye boyunca kullanamazsin!|cFFd0ffcc|H|h"}; // Get message when you use time so fast
		...
		if (pmsg2->Get()->uiNumRows > 0) {
			...
			if (atoi(row[0]) > 0) {
				if (GetNewPetSystem()->IsActivePet()) {
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Senin zaten besledigin bir pet var."));
					return false;
				}

				DBManager::instance().DirectQuery("UPDATE new_petsystem SET duration =(tduration) WHERE id = %d", item2->GetID());
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Sizin pet'in yasi artik doldu."));
			}
			else {
				DBManager::instance().DirectQuery("UPDATE new_petsystem SET duration =(tduration/2) WHERE id = %d", item2->GetID());
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Sizin pet'in yasi artik yenilenebilir."));
			}
			item->SetCount(item->GetCount() - 1);
			return true;
		}
		...
		if (item2 = GetItem(DestCell)) {
			if (item2->GetVnum() == 55002)
			{
				...
				else
				{
					...
					item2->SetForceAttribute(14, 1, item2->GetID());
		...
		case ITEM_CAMPFIRE:
			{
				...
				{
					ChatPacket(CHAT_TYPE_INFO, "<Sistem> Bunu yapabilmek icin 5 saniye beklemelisin!");
					return false;
				}
					...
					case USE_SPECIAL:
						...
						switch (item->GetVnum())
						{
							...
							case 27996: // ¥ì¢Ò¨¬¢¥
								{
									...
									if (pMap != NULL)
									{
										...
										if (f.m_mapStone.size() > 0)
										{
											...
											{
												ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Bu mapta boss bulunmamakta!"));
											}
										}
										else
										{
											ChatPacket(CHAT_TYPE_INFO, ("Bu mapta boss bulunmamakta!"));
										}
							...
							case 27124: // New Item Bandage for Bleed
								RemoveBleeding();
								break;
#ifdef __EXTRA_FUNCTION_OPEN_TELEPORT__
							case 70058:
								ChatPacket(CHAT_TYPE_INFO,"<Bilgi> Isinlanma penceresini açtýn.");
								ChatPacket(CHAT_TYPE_COMMAND, "OpenSTeleportGUI");
								break;
#endif
							case 27987: // Á¶°³
								// 50  µ¹Á¶°¢ 47990
							...
							case 80040:
								{
									int iWon = 20;
									ITEM_MANAGER::instance().RemoveItem(item);
									ChatPacket(CHAT_TYPE_INFO, LC_TEXT("20 Won envanterine yuklendi."));
							...
							case 80041:
								{
									...
									if (weapon)
									{
										...
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Silahina 1000 evrim puani yuklendi."));
									}
									else
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Silahin giyili olmadigi icin evrim puani yukleyemedim."));
									}
								}
								break;
							...
							case 80042:
								{
									...
									if (weapon)
									{
										...
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Silahina 6000 evrim puani yuklendi."));
									}
									else
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Silahin giyili olmadigi icin evrim puani yukleyemedim."));
...
//										return false;
                                    ...
                                    if (item2->IsEquipped())
                                    {
                                        ChatPacket(CHAT_TYPE_INFO, LC_TEXT("De?þtiremeyece?n bir maddenin bonus donat?mýþ!"));
                                        return false;
                                    }
                                    ...
                                    if (item2->GetType() == ITEM_WEAPON)
                                    {
                                        ...
                                        ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Silahina bu bonus ekleyemezsiniz!"));
                                        return false;
                                    ...
                                    if (item2->GetType() == ITEM_ARMOR)
                                    {
                                        ...
                                        ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Bu madde uzerinde bu bonus ekleyemezsiniz!"));
                                        return false;
                                    ...
                                    if (item2->GetType() == ITEM_BELT)
                                    {
                                        ...
                                        ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Kemere bu bonus ekleyemezsiniz!"));
                                        return false;
                                    ...
                                    if (item2->GetSubType() == COSTUME_ACCE)
                                    {
                                        ...
                                        ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Kusak bu bonus ekleyemezsiniz!"));
                                        return false;
									...
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Bu nesne icerinde bir bonus ekleyebilirsiniz."));
										return false;
									}

									if (item2->AddRareAttribute() == true)
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Taki basarili bir bonus eklendi!"));
									...
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Bu madde icerinde bir bonus ekleyemezsiniz!"));
									}
								}
								break;
							case 72325 ://matkap 4.delik zirh
								...
								case 72720 :
								{
									...
									if (item2->GetType() == ITEM_ARMOR && item2->GetSubType() == ARMOR_BODY)
									{
										...
										{
											ChatPacket(CHAT_TYPE_INFO, "Bu ekipman uygun degil.");
											return false;
										}
										else if (item2->GetSocketCount() == ITEM_SOCKET_MAX_NUM)
										{
											ChatPacket(CHAT_TYPE_INFO, "En fazla 4 tas ekleyebilirsin.");
											return false;
										}
										item2->SetSocket(3,1);
										ChatPacket(CHAT_TYPE_INFO, "Soket basariyla eklendi.");
										item->SetCount(item->GetCount() - 1);
									}
									else
									{
										ChatPacket(CHAT_TYPE_INFO, "Sadece zirhta kullanabilirsin.");
										break;
									}
								}
								break;
								...
								case 72721 : 
								{
									...
									{
										ChatPacket(CHAT_TYPE_INFO, "Sadece silahlar icindir.");
										break;
									}
								}
								break;
								
							/*case 71052 : // Schimba Bonus Costum
								{
									// À¯·´, ½Ì°¡Æú, º£Æ®³² ÁøÀç°¡ »ç¿ë±ÝÁö
//									if (LC_IsEurope() || LC_IsSingapore() || LC_IsVietnam())
//										return false;
                                    ...
                                    if (item2->IsEquipped())
                                    {
                                        ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Nu poti modifica bonusul unui item echipat!"));
                                        return false;
                                    }
                                    ...
                                    if (item2->GetType() == ITEM_WEAPON)
                                    {
                                        ...
                                        ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Nu poti schimba acest bonus pe arma!"));
                                        return false;
                                    ...
                                    if (item2->GetType() == ITEM_ARMOR)
                                    {
                                        ...
                                        ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Nu poti schimba acest bonus pe acest item!"));
                                        return false;
                                    ...
                                    if (item2->GetType() == ITEM_BELT)
                                    {
                                        ...
                                        ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Nu poti schimba acest bonus pe curea!"));
                                        return false;
                                    ...
                                    if (item2->GetSubType() == COSTUME_ACCE)
                                    {
                                        ...
                                        ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Nu poti schimba acest bonus pe esarfa!"));
                                        return false;
									...
									if (item2->GetAttributeSetIndex() == -1)
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Bu nesnenin bonus degistirebilirsiniz."));
										return false;
									}
									...
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Takin basariyla efsunu degistirdi!"));
									...
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Bu oge icerinde bonus degistirebilirsiniz!"));
									}
								}
								break;*/
							...
							case FUCKING_BRAZIL_ITEM_AUTO_HP_RECOVERY_S:
								{
									...
									#ifdef __GF_UPDATE_CHINESE_TEMPLE__
									{
										quest::CQuestManager& q = quest::CQuestManager::instance();
										quest::PC* pPC = q.GetPC(GetPlayerID());
										...
										if (pPC != NULL)
										{
											...
											{
												ChatPacket(CHAT_TYPE_INFO, "<Sistem> Bunu yapabilmek icin %d saniye beklemelisin.", 3 - (get_global_time() - last_use_time));
												return false;
											}
					...
					case USE_ADD_ATTRIBUTE_COSTUME:
						{
							LPITEM item2;
							if (!IsValidItemPosition(DestCell) || !(item2 = GetItem(DestCell)))
								return false;
							...
							int new_count = attrs;
							int precent = number(1,100);
							if(precent > 70)//here you can set change of adding new attr
								new_count = new_count + 1;
							for(int i=0;i<=ITEM_ATTRIBUTE_MAX_NUM;i++)
								item2->SetForceAttribute(i, 0, 0);
							int type1=0,type2=0,type3=0,value1=0,value2=0,value3=0;
							if(new_count == 1){
								type1=number(1,41);
								value1=number(1,20);//set min max value of attr
								if(type1 == 1 || type1 == 2)
									value1 = value1*100;
								item2->SetForceAttribute(0,type1,value1);								
							}
							else if(new_count == 2){
								value1=number(1,20);//set min max value of attr
								value2=number(1,20);//set min max value of attr
								while ( type1 == type2)
								{
									type1=number(1,41);
									type2=number(1,41);
								...
								if(type2 == 1 || type2 == 2)
									value2 = value2*100;
								item2->SetForceAttribute(0,type1,value1);
								item2->SetForceAttribute(1,type2,value2);
								...
								value2=number(1,20);//set min max value of attr
								type3=number(1,41);
								value3=number(1,20);//set min max value of attr
								while ( type1 == type2 || type2==type3 || type1==type3)
								{
									type1=number(1,41);
									type2=number(1,41);
									type3=number(1,41);
								...
								item2->SetForceAttribute(0,type1,value1);
								item2->SetForceAttribute(1,type2,value2);
								item2->SetForceAttribute(2,type3,value3);
							}
							item->SetCount(item->GetCount() - 1);
						}
						break;
					case USE_CHANGE_ATTRIBUTE_COSTUME:
						{
							LPITEM item2;
							if (!IsValidItemPosition(DestCell) || !(item2 = GetItem(DestCell)))
								return false;
							...
							if(item2->GetSubType() != COSTUME_BODY)
								return false;
							int attrs = item2->GetAttributeCount();
							//ChatPacket(CHAT_TYPE_INFO, "attrs %d",attrs);
							for(int i=0;i<=ITEM_ATTRIBUTE_MAX_NUM;i++)
								item2->SetForceAttribute(i, 0, 0);
							int type1=0,type2=0,type3=0,value1=0,value2=0,value3=0;
							if(attrs == 1){
...
/*#ifdef __USE_TESTSERVER__
					...
					case USE_ADD_ATTRIBUTE2:
						{
							...
							for (int i=0 ; i < ITEM_LIMIT_MAX_NUM ; i++)
							{
								if (LIMIT_REAL_TIME == table->aLimits->bType)
								{
									ChatPacket(CHAT_TYPE_INFO, "Sureli silaha efsun basamazsin.");
									return false;
								}
							}
					...
					/*if (item->GetVnum() == 80039)
					{
						if (item->IsDragonSoul())
						{
							...
							item->SetForceAttribute(1, 0, 0);
							item->SetForceAttribute(2, 0, 0);
							item->SetForceAttribute(3, 0, 0);
							item->SetForceAttribute(4, 0, 0);
							item->SetForceAttribute(5, 0, 0);
							item->SetForceAttribute(6, 0, 0);
					...
					else
					{
						if (item->IsDragonSoul())
						{
							return DSManager::instance().ExtractDragonHeart(this, item, item);
						}
					}
					return false;*/
								...
								case USE_ADD_ATTRIBUTE :
									...
									if (item2->IsEquipped())
									{
										ChatPacket(CHAT_TYPE_INFO, "Esya uzerinizdeyken islem yapamazsiniz.");
										return false;
									}
					...
					case USE_COSTUME_TRANSFORM:
						{
							...
							switch (item->GetSubType())
							{
								case USE_COSTUME_ENCHANT:
									{
										int aiChangeProb[ITEM_ATTRIBUTE_MAX_LEVEL] = {0, 10, 40, 35, 50};
										item2->ChangeAttribute(aiChangeProb);
...
bool CHARACTER::UseItem(TItemPos Cell, TItemPos DestCell)
{
	...
	if (ITEM_BELT == item->GetType())
		ERROR_MSG(CBeltInventoryHelper::IsExistItemInBeltInventory(this), "Kemer sekmesindeki itemlerinizi alýnýz.");
	//sys_log(0, "%s: USE_ITEM %s (inven %d, cell: %d)", GetName(), item->GetName(), window_type, wCell);
#ifdef __BIOLOG_SYSTEM___TIME
...
#ifdef __GROWTH_PET_SYSTEM__
	if (GetNewPetSystem()->IsActivePet() && item->GetVnum() >= 55701 && item->GetVnum() <= 55704)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Once petini gonder. "));
		return false;
	}
#endif	
		
	//if (ch->GetNewPetSystem()->IsActivePet() && item->GetVnum() >= 55701 && item->GetVnum() <= 55704)
	if (item->GetVnum() >= 71221 && item->GetVnum() <= 71228)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Bineklere islem yapamazsin! "));
		return false;
	}
		
	//if (ch->GetNewPetSystem()->IsActivePet() && item->GetVnum() >= 55701 && item->GetVnum() <= 55704)
	/*if (ch->GetRaceNum() >= 20201 && ch->GetRaceNum() <= 20260)
	{
		ChatPacket(CHAT_TYPE_INFO, "Bineklere islem yapamazsin! ");
		return ;
	}*/
...
bool CHARACTER::EquipItem(LPITEM item, int iCandidateCell)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Bu islemi bu kadar hizli yapamassin!"));
		return false;
	}
...
void CHARACTER::AutoRecoveryItemProcess(const EAffectTypes type)
{
	...
	if (AFFECT_AUTO_HP_RECOVERY == type && (GetDuel("BlockPotion")))
		// RemoveAffect(AFFECT_AUTO_HP_RECOVERY); test o sa testez la mine sa il si dezactiveze ca acum iti opreste regenerarea doar el ramane activat animatia dar nu iti baga pv cu asta compileaza.
		return;
#endif
