...
namespace quest
{
	...
	int _get_table_postfix2(lua_State* L)
	{
		lua_pushstring(L, get_table_postfix());
		return 1;
	}

	int _mysql_direct_query2(lua_State* L)
	{
		// char szQuery[1024];
	...
	int _mysql_escape_string2(lua_State* L)
	{
		char szQuery[1024] = {0};
	...
	struct FKillSectree2
	{
		void operator () (LPENTITY ent)
		{
			if (ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (!ch->IsPC() && !ch->IsPet() && !ch->IsNewPet() && !ch->IsNewMount())
#else
				if (!ch->IsPC() && !ch->IsPet())
#endif
					ch->Dead();
		...
		void operator () (LPENTITY ent)
		{
			if (true == ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (pChar == ExceptChar)
					return;
#ifdef __GROWTH_PET_SYSTEM__
				if (!pChar->IsPet() && !pChar->IsNewPet() && !pChar->IsNewMount() && (true == pChar->IsMonster() || true == pChar->IsStone()))
#else				
