...
void CInputDB::PlayerLoad(LPDESC d, const char * data)
{
	...
	for (int i = 0; i < NEWQUICKSLOT_MAX_NUM; ++i)
		ch->SetNewQuickslot(i, pTab->newquickslot[i]);
#endif
	ch->PointsPacket();
	ch->SkillLevelPacket();
...
void CInputDB::SafeboxLoad(LPDESC d, const char * c_pData)
{
	...
	if (ch->GetShopOwner() || ch->GetExchange() || ch->GetMyShop() || ch->IsCubeOpen() || ch->IsAcceOpen() || ch->GetOfflineShopOwner())
	{
