...
void CNewPetActor::ItemCubeFeed(int type)
{
	for (int i = 0; i < 9; ++i) {
		if (m_dwpetslotitem[i] != -1) {
			...
			if (!itemxp)
				return;
			if (itemxp->GetID() == ITEM_MANAGER::instance().FindByVID(this->GetSummonItemVID())->GetID() || m_pkOwner->GetExchange() || m_pkOwner->GetMyShop() || m_pkOwner->GetShopOwner() || m_pkOwner->IsOpenSafebox() || m_pkOwner->IsCubeOpen() || m_pkOwner->IsAcceOpen()  || m_pkOwner->IsAttrTransferOpen())
				return;
			if(type == 1)
			{
			...
			else if(type == 3)
			{
				if(itemxp->GetType() == 1 || itemxp->GetType() == 2)
				{
					if (GetNextExpFromItem() - GetExpI() > 0)
					{
						...
						m_pkOwner->ChatPacket(CHAT_TYPE_INFO, "Daha fazla item-exp veremezsiniz.");
                }
            }
		}
	}
...
bool CNewPetActor::IncreasePetSkill(int skill) {
	...
	for (int i = 0; i < 3; ++i) { //Itero gli slot per cercare la skill
		if (m_dwskillslot[i] == skill) {  //Se trova la skill o la aumenta oppure e' gi?max
			if (m_dwskill[i] < 20) {
				m_dwskill[i] += 1;
				m_pkOwner->ChatPacket(CHAT_TYPE_INFO, "Beceri duzeyine ulasti:%d", m_dwskill[i]);
	...
	*/
	m_pkOwner->ChatPacket(CHAT_TYPE_INFO, "Evcil hayvaninizin diger becerileri ogrenmek mumkun degil!");
	return false;
}
...
bool CNewPetActor::IncreasePetEvolution() {
	if (m_dwevolution < 3) {
		...
		m_pkOwner->ChatPacket(CHAT_TYPE_INFO, "Evcil hayvan maksimum evrime ulasti!");
		return false;
	}
	return true;
}
...
bool CNewPetActor::Update(DWORD deltaTime)
{
	...
	if (m_pkOwner->IsDead() || (IsSummoned() && m_pkChar->IsDead()) || (IsSummoned() && (m_pkOwner->GetExchange() || m_pkOwner->GetShop() || m_pkOwner->GetMyShop() || m_pkOwner->GetShopOwner() || m_pkOwner->IsOpenSafebox() || m_pkOwner->IsCubeOpen() || m_pkOwner->IsAcceOpen()  || m_pkOwner->IsAttrTransferOpen() || m_dwduration <= 0))
