...
namespace quest
{
	...
	int item_get_evolution(lua_State* L)
	{
		CQuestManager& q = CQuestManager::instance();
		LPITEM item = q.GetCurrentItem();

		if (item)
			lua_pushnumber(L, item->GetEvolution());
		else
			lua_pushnumber(L, 0);
	...
	int item_set_evolution(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		LPITEM item = CQuestManager::instance().GetCurrentItem();
		...
		item->SetEvolution(count);
	}
	
	int item_is_sealed(lua_State* L)
	{
		CQuestManager& q = CQuestManager::instance();
	...
	void RegisterITEMFunctionTable()
	{
		...
		luaL_reg item_functions[] =
		{
			...
			{ "get_evolution",				item_get_evolution					},
			{ "set_evolution",				item_set_evolution					},	
