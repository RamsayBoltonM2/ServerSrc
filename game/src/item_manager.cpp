...
void ITEM_MANAGER::RemoveItem(LPITEM item, const char * c_pszReason)
{
	...
	if ((o = item->GetOwner()))
	{
		...
		else
		{
			...
			o->SyncNewQuickslot(QUICKSLOT_TYPE_ITEM, item->GetCell(), 1000);
#endif
...
void ITEM_MANAGER::CreateQuestDropItem(LPCHARACTER pkChr, LPCHARACTER pkKiller, std::vector<LPITEM> & vec_item, int iDeltaPercent, int iRandRange)
{
	...
	if (pkChr->GetLevel() >= 30 && (GetDropPerKillPct(50, 100, iDeltaPercent, "ds_drop") >= number(1, iRandRange)))
	{
		...
		pkKiller->AutoGiveItem (dragon_soul_gemstone, 1);
	}

	if (pkChr->GetLevel() >= 5 && (GetDropPerKillPct(50, 100, iDeltaPercent, "sihastru") >= number(1, iRandRange)))
	{
		const static DWORD upp_sihastru_item = 30254;
		if ((item = CreateItem(upp_sihastru_item, 1, 0, true)))
			vec_item.push_back(item);
	}
