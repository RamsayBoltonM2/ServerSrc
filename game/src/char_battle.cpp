...
#ifdef __GF_UPDATE_STROKE_INFO__
...
enum EdecltypeKills
{
	INCREASE_KILLS_PLAYER = 1,
	INCREASE_KILLS_MOB = 2,
};

void IncreaseKills(LPCHARACTER ch, LPCHARACTER tch, int mode)
{
	char buf[CHAT_MAX_LEN + 1];
	
	if (mode == INCREASE_KILLS_PLAYER)
		snprintf(buf, sizeof(buf), "player.count_kills");
	else if (mode == INCREASE_KILLS_MOB)
		snprintf(buf, sizeof(buf), "mob.count_kills");	

	int numberCount, total;

	numberCount = ch->GetQuestFlag(buf);
	total = numberCount += 1;
	ch->SetQuestFlag(buf, total);
}
#endif
	...
	if (bDropInventory) // Drop Inventory
	{
		...
		if (!vec_bSlots.empty())
		{
			...
			for (i = 0; i < iQty; ++i)
			{
				...
				SyncNewQuickslot(QUICKSLOT_TYPE_ITEM, vec_bSlots[i], 1000);
#endif
...
#ifdef __SECONDARY_SLOT_SYSTEM__
			SyncNewQuickslot(QUICKSLOT_TYPE_ITEM, WEAR_UNIQUE1, 1000);
#endif
			vec_item.push_back(std::make_pair(pkItem->RemoveFromCharacter(), EQUIPMENT));
		}
...
#ifdef __SECONDARY_SLOT_SYSTEM__
			SyncNewQuickslot(QUICKSLOT_TYPE_ITEM, WEAR_UNIQUE2, 1000);
#endif
			vec_item.push_back(std::make_pair(pkItem->RemoveFromCharacter(), EQUIPMENT));
		}
	}
...
void CHARACTER::Dead(LPCHARACTER pkKiller, bool bImmediateDead)
{
	...
	if (pkKiller && pkKiller->IsPC())
	{
		...
		{	
			...
			int betMoney = GetQuestFlag(szTableStaticPvP[8]);
			
			if (betMoney > 0) {
				pkKiller->PointChange(POINT_GOLD, betMoney*2, false);
				pkKiller->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("duel_reward_money_win"), betMoney);
			}
		...
		if (!IsPC() && pkKiller->IsBlockMode(BLOCK_POINT_EXP))
		{
			if (pkKiller->GetQuestFlag("karakter.moboldurme") >= 1000)
				pkKiller->SetQuestFlag("karakter.dondur",1);
			else
			pkKiller->SetQuestFlag("karakter.moboldurme",pkKiller->GetQuestFlag("karakter.moboldurme")+1);
		}
		...
		if (IsPC())
		{
			...
			if (g1 && g2)
			{
				if (g1->UnderWar(g2->GetID()))
				{
					...
					if (pMap)
						pMap->SendKillNotice(pkKiller->GetName(), GetName(), pkKiller->GetRaceNum(), GetRaceNum());
				}
			}
	...
	if(!IsPC())
	{
		...
		{
			DBManager::instance().DirectQuery("UPDATE player SET bossbarones = bossbarones+1 WHERE name = '%s'", pkKiller->GetName());

		}
		else if (pkKiller && pkKiller->IsPC() && 2493 == GetMobTable().dwVnum)
		{
			DBManager::instance().DirectQuery("UPDATE player SET bossejder = bossejder+1 WHERE name = '%s'", pkKiller->GetName());

		}
		else if (pkKiller && pkKiller->IsPC() && 2598 == GetMobTable().dwVnum)
		{
			DBManager::instance().DirectQuery("UPDATE player SET bossazrail = bossazrail+1 WHERE name = '%s'", pkKiller->GetName());

		}
		else if (pkKiller && pkKiller->IsPC() && 1093 == GetMobTable().dwVnum)
		{
			DBManager::instance().DirectQuery("UPDATE player SET bosslusifer = bosslusifer+1 WHERE name = '%s'", pkKiller->GetName());

		}
		else if (pkKiller && pkKiller->IsPC() && 2291 == GetMobTable().dwVnum)
		{
			DBManager::instance().DirectQuery("UPDATE player SET bosskirmiziejder = bosskirmiziejder+1 WHERE name = '%s'", pkKiller->GetName());

		}
		else if (pkKiller && pkKiller->IsPC() && 6091 == GetMobTable().dwVnum)
		{
			DBManager::instance().DirectQuery("UPDATE player SET bossrazador = bossrazador+1 WHERE name = '%s'", pkKiller->GetName());

		}
		else if (pkKiller && pkKiller->IsPC() && 6191 == GetMobTable().dwVnum)
		{
			DBManager::instance().DirectQuery("UPDATE player SET bossnemere = bossnemere+1 WHERE name = '%s'", pkKiller->GetName());

		}
		else if (pkKiller && pkKiller->IsPC() && 6408 == GetMobTable().dwVnum)
		{
			DBManager::instance().DirectQuery("UPDATE player SET bossmeley = bossmeley+1 WHERE name = '%s'", pkKiller->GetName());
	...
	if (IsPC() && pkKiller)
	{
		if (pkKiller->IsPC() && GetDesc()->GetHostName() != pkKiller->GetDesc()->GetHostName())
		{
			if (GetMapIndex() == 112 && pkKiller->GetMapIndex() == 357)
			{
				if (this->GetLevel() >= 55 && this->GetLevel() <= 70)
				{
					pkKiller->PointChange(POINT_WAR_POINT, 1, true);
				}
				else if (this->GetLevel() >= 71 && this->GetLevel() <= 90)
				{
					pkKiller->PointChange(POINT_WAR_POINT, 2, true);
				}
				else if (this->GetLevel() >= 91)
				{
					pkKiller->PointChange(POINT_WAR_POINT, 3, true);
				}
			}
		}
	}
	...
	if (GetRaceNum() == 6420)
	{
		if (pkKiller->GetLevel() >= 80)
		{
			DWORD value = 0;
			if (pkKiller->GetStrokeByVid(GetVID(), &value))
			{
				pkKiller->SetQuestFlag("yildonumuparasi.yildonumupoint",pkKiller->GetQuestFlag("yildonumuparasi.yildonumupoint")+(int)value);
			}
		}
		else
		{
			pkKiller->ChatPacket(CHAT_TYPE_INFO, "Golge savascisindan puan kazanabilmek icin 80. seviye ustu olmalisiniz.");
		}
	}
	...
	if (pkKiller && IsPC())
	{
		...
		else
		{
			...
			{
				if (!isAgreedPVP && !isUnderGuildWar && !IsKillerMode() && GetAlignment() >= 0 && !isDuel && !isForked && pkKiller->GetMapIndex() != 200 && pkKiller->GetMapIndex() != 218)
				{
	...
	{
		IncreaseKills(pkKiller, this, INCREASE_KILLS_PLAYER);	
	}
	if ((pkKiller && pkKiller->IsPC()) && !IsPC())
	{
		IncreaseKills(pkKiller, this, INCREASE_KILLS_MOB);	
	}
#endif
...
bool CHARACTER::Damage(LPCHARACTER pAttacker, int dam, EDamageType type) // returns true if dead
{
    ...
    if (GetRaceNum() == 20399)
    {
       if (pAttacker->GetJob() == JOB_WARRIOR)
       {
          SendDamagePacket(pAttacker, 0, DAMAGE_BLOCK);
          return false;
       }
       
       if (pAttacker->GetJob() == JOB_SURA)
       {
          SendDamagePacket(pAttacker, 0, DAMAGE_BLOCK);
          return false;
       }
	...
	if (pAttacker)
	{
...
#if defined(__YMIR_UPDATE_WOLFMAN__) && !defined(USE_MOB_BLEEDING_AS_POISON)
		else if (type == DAMAGE_TYPE_BLEEDING)
			damageFlag = DAMAGE_BLEEDING;
#elif defined(__YMIR_UPDATE_WOLFMAN__) && defined(USE_MOB_BLEEDING_AS_POISON)
		else if (type == DAMAGE_TYPE_BLEEDING)
			damageFlag = DAMAGE_POISON;
#endif
		else
		...
		if (pAttacker)
		{
			...
			if (IsRaceInfo(GetRaceNum()) && pAttacker->IsPC())
