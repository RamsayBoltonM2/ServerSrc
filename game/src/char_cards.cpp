...
{
	if (GetExchange() || GetMyShop() || GetShopOwner() || IsOpenSafebox() || IsAcceOpen() || IsCubeOpen() || IsAttrTransferOpen())
	{
	...
	if (character_cards.cards_left <= 0) 
	{
		if (GetGold() < 30000)
		{
			ChatPacket(CHAT_TYPE_INFO, "Yeterli Paraniz Yok.");
			return;
		}
		if (CountSpecifyItem(79506) < 1)
		{
			ChatPacket(CHAT_TYPE_INFO, "Gerekli Olan kart Yok.");
			return;
		}
...
void CHARACTER::Cards_pullout()
{
	...
	if (empty_space == i)
	{
		ChatPacket(CHAT_TYPE_INFO, "Kart koyabilmek icin bos yer yok.");
		return;
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, "Masada kart koymak icin yer yok.");
		return;
	}
	RandomizeCards();
	SendUpdatedInformations();
...
void CHARACTER::CardsEnd()
{
	...
	DBManager::instance().DirectQuery("INSERT INTO okay_event%s (pid, name, points) VALUES (%d, '%s', %d)", get_table_postfix(), GetPlayerID(), GetName(), character_cards.points);
	Cards_clean_list();
	SendUpdatedInformations();
}
...
{
	std::unique_ptr<SQLMsg> pMsg(DBManager::Instance().DirectQuery("SELECT name, SUM(points) as spoints FROM okay_event%s GROUP BY pid ORDER BY spoints DESC LIMIT 10", get_table_postfix()));
...
{
	std::unique_ptr<SQLMsg> pMsg(DBManager::Instance().DirectQuery("SELECT name, points FROM okay_event%s ORDER BY points DESC LIMIT 10", get_table_postfix()));
	if (pMsg->Get()->uiNumRows == 0)
	{
		return;
	}
