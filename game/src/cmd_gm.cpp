...
ACMD(do_delete_tickets_closed)
{
	...
	if (!strcmp(arg1, "true"))
	{
		std::unique_ptr<SQLMsg> dwExtract(DBManager::instance().DirectQuery("SELECT COUNT(*) FROM ticket.list WHERE status = 2"));
...
ACMD(do_mountfeed)
{
	...
	{
		ch->FeedHorse();
		ch->ChatPacket(CHAT_TYPE_INFO, "<Debug> Succes.");
	}
	return;
}
...
struct FuncDead
{
	...
	FuncDead(LPCHARACTER ch) : m_pkGM(ch)
	{
	}
...
ACMD(do_kill_players)
{
	...
	if (sectree)
		...
		sys_err("KILL_ERROR.NULL_SECTREE(mapIndex=%d, pos=(%d, %d)", ch->GetMapIndex(), ch->GetX(), ch->GetY());
}
#endif
...
struct FuncPurge
{
	...
	void operator () (LPENTITY ent)
	{
		...
		if (pkChr->GetRaceNum() != 30000 && pkChr->IsNPC() && !pkChr->IsPet() && !pkChr->IsNewMount() && !pkChr->IsNewPet() && pkChr->GetRider() == NULL)
#else
...
ACMD(do_item_purge)
{
	...
	for (i = 0; i < INVENTORY_AND_EQUIP_SLOT_MAX; ++i)
	{
		if ((item = ch->GetInventoryItem(i)))
		{
			...
			ch->SyncNewQuickslot(QUICKSLOT_TYPE_ITEM, i, 1000);
#endif
		}
	}
...
ACMD(do_anyone_teleporter)
{
	...
	if (!strcmp(arg1, "ask"))
	{
		LPCHARACTER enemy = CHARACTER_MANAGER::instance().FindPC(arg2);

		if(enemy != NULL)
		{
			if (enemy->IsGM()) {
				ch->ChatPacket(CHAT_TYPE_NOTICE, "Bir moderatore isinlanamazsiniz.");
				return;
			}
			
			ch->SetQuestFlag("teleport.ask_active", 1);
			
			enemy->ChatPacket(CHAT_TYPE_INFO, "Oyuncu %s size isinlanmak istiyor.", ch->GetName());
			enemy->ChatPacket(CHAT_TYPE_COMMAND, "teleport_to %s", ch->GetName());
		}
	}
	else if (!strcmp(arg1, "accept"))
	{
		...
		{
			int x = 0, y = 0;
			int cost = 10000000; // 10.000.000 Yang
			int blockedMaps[] = { 
				66, 70, 71, 72, 73, 79, 80, 81, 83, 85, 112, 113, 189, 110, 111, 193, 195, 353, 104, 71, 193, 66, 206, 301, 222, 307, 219, 218, 220, 307, 354, 209, 210, 207, 115 
			};
		
			quest::CQuestManager& q = quest::CQuestManager::instance();
			quest::PC* pPC = q.GetPC(enemy->GetPlayerID());
			
			if (pPC != NULL)
			{
				int last_use_time = pPC->GetFlag("teleport.last_use");
			
				if (get_global_time() - last_use_time < 60)
				{
					enemy->ChatPacket(CHAT_TYPE_INFO, "Hala %d saniye beklemeniz gerekiyor.", 60 - (get_global_time() - last_use_time));
					return;
				}
	
					for(int i = 0; i < sizeof(blockedMaps)/sizeof(blockedMaps[0]); i++)
					{
						if(blockedMaps[i] == ch->GetMapIndex()){
							enemy->ChatPacket(CHAT_TYPE_NOTICE, "Hedef engellenen bir harita uzerindedir.");
							return;
						}
					}
					
					if(enemy->GetQuestFlag("teleport.ask_active") == 1)
					{
						enemy->SetQuestFlag("teleport.ask_active", 0);
						//continue;
					}
					else
					{
						enemy->ChatPacket(CHAT_TYPE_NOTICE, "Dc plm bunu yapmak istiyorsun?.");
						return;
					}
					
					#ifdef __CANBLOCK_PLAYER_SYSTEM__
					if (ch->isBlocked(enemy->GetName()))
					{
						enemy->ChatPacket(CHAT_TYPE_INFO, "%s yonlendirilemezsiniz cunku sizi engellemistir.", ch->GetName());
						return;
					}
					#endif
				
					if (enemy->IsDead()) {
						enemy->ChatPacket(CHAT_TYPE_NOTICE, "Olduysen isinlanamazsiniz.");
						return;
					}
					
					if (ch->IsDead()) {
						enemy->ChatPacket(CHAT_TYPE_NOTICE, "Hedef oldü.");
						return;
					}
					
					if (ch->IsGM()) {
						enemy->ChatPacket(CHAT_TYPE_NOTICE, "Bir moderatore isinlanamazsiniz.");
						return;
					}
					
					if (enemy->GetGold() < cost)
					{
						enemy->ChatPacket(CHAT_TYPE_INFO, "Bunu yapmak icin %d yang'a ihtiyacýnýz var.", cost);
						return;
					}
					
					const CCI* pkCCI = P2P_MANAGER::instance().Find(arg2);
					if (NULL != pkCCI && pkCCI->bChannel != g_bChannel)
					{
						enemy->ChatPacket(CHAT_TYPE_INFO, "Hedef (%s) kanalinda %d kanal (kanalim %d)", arg2, pkCCI->bChannel, g_bChannel);
						return;
					}
				...
				pPC->SetFlag("teleport.last_use", get_global_time());
	...
	else if (!strcmp(arg1, "cancel"))
	{
		LPCHARACTER enemy = CHARACTER_MANAGER::instance().FindPC(arg2);

		if(enemy != NULL)
		{
			enemy->ChatPacket(CHAT_TYPE_INFO, "Aktarimi reddetti.");
		}
	}
}
#endif
...
ACMD(do_guild_notice)
{
	if (ch->GetGuild() && ch->GetGuild()->GetMasterPID() == ch->GetPlayerID())
	{
		...
		DBManager::instance().Query(
			"INSERT DELAYED INTO `log`.`guild_notice%s` (`sender_pid`, `time`, `ip`, `content`, `channel`) "
			"VALUES (%u, NOW(), '%s', '%s', %d);",
...
ACMD(do_who)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "Total [%d] %d / %d / %d (this server %d)", 
			iTotal+2000, paiEmpireUserCount[1]+2000, paiEmpireUserCount[2]+2000, paiEmpireUserCount[3]+2000, iLocal);
}
...
#endif
		case 10: // national point
			{
				long amount = 0;
				str_to_number(amount, arg3);
				tch->PointChange(POINT_WAR_POINT, amount, true);
			}
			break;
		...
		case 11:
			{
				...
				if (weapon)
				{
					weapon->SetEvolution(weapon->GetEvolution()+amount);
				}
			}
			break;
		...
		case 13:
			{
				int	amount = 0;
				str_to_number(amount, arg3);
				tch->UpdateTitle(amount - ch->GetTitle());
			}
			break;
	}
	...
	if (*arg1)
	{
		switch (LOWER(*arg1))
		{
			...
			case 'a':
				...
				//END_RELOAD_ADMIN
			case 'c':
				ch->ChatPacket(CHAT_TYPE_INFO, "Cube tablosu yenileniyor.");
				Cube_init();
				break;
			case 'acce':
				ch->ChatPacket(CHAT_TYPE_INFO, "Acce tablosu yenileniyor.");
				Acce_init();
				break;
		}
	}
...
{
	if (g_bChannel != 1) {
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("OFFLINE_SHOP_GUI_OPEN_FAILED"));
		return;
	}
