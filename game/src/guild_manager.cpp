...
DWORD CGuildManager::CreateGuild(TGuildCreateParameter& gcp)
{
    ...
    {
        gcp.master->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<e¸¸ea€œA“> e¸¸ea€œA“ i씊쩯|a€엍씊� i 걁a€￠ⓒia€￠Eœi§a?￢ ia€￠A iA μea€¹E†ea€¹¤."));
        return 0;
    }
    ...
    if (pmsg->Get()->uiNumRows > 0)
    {
        ...
        if (!(row[0] && row[0][0] == '0'))
        {
            gcp.master->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<e¸¸ea€œA“> i씊쩯?¸ e°a?￠i쓆?￢ i씊쩯|a€엍쒮œ e¸¸ea€œA“e°a?￢ i얝†iA μea€¹E†ea€¹¤."));
            return 0;
        }
    }
    else
    {
        gcp.master->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<e¸¸ea€œA“> e¸¸ea€œA“e￥¼ iÆ’쓎a€옟푝a€￠  iE†Eœ ia€”a€ iA μea€¹E†ea€¹¤."));
        return 0;
    }
